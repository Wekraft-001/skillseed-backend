...
export class RewardsService {
  constructor(
    ...
    @InjectModel(EducationalContent.name) private educationalContentModel: Model<EducationalContentDocument>,
  ) {}
  ...
  async completeEducationalContent(
    ...
    const educationalContent = await this.educationalContentModel.findById(educationalContentId).exec();
    ...
    } else if (contentType === 'book' && educationalContent.books[contentIndex]) {
      title = educationalContent.books[contentIndex].title;
    } else if (contentType === 'game' && educationalContent.games[contentIndex]) {
      title = educationalContent.games[contentIndex].name;
    } else {
      throw new NotFoundException(`${contentType} at index ${contentIndex} not found`);
    }

    // Check if star already exists
    let star = await this.starModel.findOne({ 
      user: userId, 
  ...
  private getBadgeTypeFromChallenge(challenge: Challenge): string {
    ...
    if (categoryName?.includes('science')) {
      ...
      return 'reading';
    } else if (categoryName?.includes('cod') || categoryName?.includes('program')) {
      return 'coding';
    } else {
      return 'general';
    }
  ...
  private getBadgeDescription(badgeType: string): string {
    ...
    const badgeDescriptions = {
      'science': 'Completed a science challenge and demonstrated scientific curiosity',
      'math': 'Mastered mathematical concepts and solved challenging problems',
      'reading': 'Demonstrated excellent reading comprehension and literary analysis',
      'coding': 'Wrote functioning code and demonstrated programming skills',
      'general': 'Successfully completed a learning challenge'
    };
    
    return badgeDescriptions[badgeType] || 'Successfully completed a learning challenge';
  }
  ...
  async checkAndAwardTierBadges(userId: string): Promise<void> {
    ...
    const stars = await this.starModel.find({ user: userId, completed: true }).exec();
    const totalStars = stars.length;
    const completedActivities = new Set(stars.map(s => s.educationalContent.toString())).size;
  ...
  async checkAndAwardSpecialBadges(userId: string): Promise<void> {
    ...
    const badges = await this.badgeModel.find({ user: userId, isCompleted: true }).exec();
